...
void main() {
    // ambient
    float ambientStrengh = 0.4; // Set ambient strength
    ...
    // specular
    float specularStrength = 0.5f; // Sets specularStrength
    vec3 viewDir = normalize(viewPos - FragPos); // Gets viewDir
    vec3 reflectDir = reflect(-lightDir, norm); // Gets reflectDir
    float spec = pow(max(dot(viewDir, reflectDir), 0.8), 32); // Gets spec
