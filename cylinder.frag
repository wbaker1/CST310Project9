...
{
    // ambient
    float ambientStrength = 0.4;  // Set ambient strength
    ...
    // specular
    float specularStrength = 0.5f;  // Sets specular strength
    vec3 viewDir = normalize(viewPos - FragPos);  // Sets view direction
    vec3 reflectDir = reflect(-lightDir, norm);  // Sets reflect direction
    float spec = pow(max(dot(viewDir, reflectDir), 0.4), 32);  // Sets specular based on power, max, and dot product
